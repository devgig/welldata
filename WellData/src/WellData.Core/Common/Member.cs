...
{
    // from http://jeffhandley.com/archive/2010/04/10/memberof.aspx
    ...
    public class Member
    {
        ...
        public static Member Of<T, K>(Expression<Func<T, K>> member)
        {
            return member.AsMemberOrMethod();
        }

        public static Member Of<T>(Expression<Func<T>> expression)
        {
            var lambdaExpression = (LambdaExpression)expression;
            var member = (!(lambdaExpression.Body is UnaryExpression) ? (MemberExpression)lambdaExpression.Body : (MemberExpression)((UnaryExpression)lambdaExpression.Body).Operand).Member;
